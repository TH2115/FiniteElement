...
int main(int argc, char *argv[]) {
  ...
  MPI_Bcast(K_EE, sideDimT * sideDimT, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Bcast(K_EF, sideDimT * (nDof - sideDimT), MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Bcast(K_FF, (nDof - sideDimT) * (nDof - sideDimT), MPI_DOUBLE, 0,
            MPI_COMM_WORLD);
  MPI_Bcast(T_E, (sideDimT), MPI_DOUBLE, 0, MPI_COMM_WORLD);
  delete[] TempNode;
  ...
  if (my_rank == 0) {
    ...
    MPI_Send(T_F, nDof - sideDimT, MPI_DOUBLE, 1, 5, MPI_COMM_WORLD);
    ...
    MPI_Recv(T_F, nDof - sideDimT, MPI_DOUBLE, 0, 5, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    ...
    cblas_dgemv(CblasRowMajor, CblasNoTrans, sideDimT, nDof - sideDimT, 1.0,
                K_EF, nDof - sideDimT, T_F, 1, 0.0, K_EFdotT_F, 1);
