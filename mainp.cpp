...
int main(int argc, char* argv[])
{
    ...
    opts.add_options()
        ("case", po::value<int>()->default_value(1),
				 "Case number")
        ...
        ("q_BC_side",  po::value<char>()->default_value('R'),
    ...
    int N_NodeDof[4] = {1,1,1,1}; // number of DoF per node (1 = temperature only)
    int N_eDof = 0;
    for (int i = 0; i < 4; i++){
...
///////////////////calculation of nodel coordinate matrix /////
    ...
    createLinspace(x ,0, L, N_elx + 1); //fill x with a linearly space numbers between 0 and L
